...
from verl.workers.rollout.sglang_rollout.utils import SGLANG_LORA_NAME, align_dflash_reject_token_mask
...
class SGLangHttpServer:
    ...
    def __init__(
        ...
        if self.is_composed_dflash_student and self.config.load_format == "dummy":
            logger.warning(
                "Composed OPD draft rollout trains only the draft model online, so SGLang target weights "
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        if "enable_weights_cpu_backup" in [f.name for f in dataclasses.fields(ServerArgs)]:
            enable_weights_cpu_backup = (
                ...
                if (
                    ...
                    or self.is_composed_dflash_student
                )
                else False
            )
        ...
        if (
            ...
            and "enable_draft_weights_cpu_backup" in [f.name for f in dataclasses.fields(ServerArgs)]
        ):
        ...
        # Prefer symbol-based compatibility over version checks since some forks
        # move `_launch_subprocesses` into `Engine` without bumping `__version__`.
        from sglang.srt.entrypoints import http_server as sglang_http_server

        engine_cls = getattr(sglang_http_server, "Engine", None)
        if engine_cls is not None and hasattr(engine_cls, "_launch_subprocesses"):
            ...
            launch_subprocesses = getattr(sglang_http_server, "_launch_subprocesses", None)
            if launch_subprocesses is None:
                raise ImportError(
                    "Unable to locate SGLang subprocess launcher. Expected either "
                    "`Engine._launch_subprocesses` or module-level `_launch_subprocesses`."
                )

            if version.parse(sglang.__version__) >= version.parse("0.5.7"):
                self.tokenizer_manager, self.template_manager, self.scheduler_info, *_ = launch_subprocesses(
                    server_args=server_args,
                    init_tokenizer_manager_func=sglang.srt.entrypoints.engine.init_tokenizer_manager,
                    run_scheduler_process_func=sglang.srt.entrypoints.engine.run_scheduler_process,
                    run_detokenizer_process_func=sglang.srt.entrypoints.engine.run_detokenizer_process,
    ...
    async def generate(
        ...
        extra_fields = {
            ...
            "sglang_completion_tokens": meta_info.get("completion_tokens", len(token_ids)),
        }
        for meta_key, extra_key in (
            ("spec_accept_length", "sglang_spec_accept_length"),
            ("spec_accept_token_num", "sglang_spec_accept_token_num"),
            ("spec_draft_token_num", "sglang_spec_draft_token_num"),
            ("spec_verify_ct", "sglang_spec_verify_ct"),
            ("decode_throughput", "sglang_decode_throughput"),
        ):
            if meta_key in meta_info:
                extra_fields[extra_key] = meta_info[meta_key]

        dflash_reject_token_mask = meta_info.get("dflash_reject_token_mask")
        if dflash_reject_token_mask is not None:
            aligned_reject_token_mask = align_dflash_reject_token_mask(
                dflash_reject_token_mask, response_len=len(token_ids)
            )
            dflash_reject_token_indices = [
                idx for idx, is_reject in enumerate(aligned_reject_token_mask) if bool(is_reject)
            ]
            extra_fields["dflash_reject_token_indices"] = dflash_reject_token_indices
            extra_fields["dflash_reject_token_mask_raw_len"] = len(dflash_reject_token_mask)
            extra_fields["dflash_reject_token_mask_aligned_len"] = len(aligned_reject_token_mask)
            extra_fields["dflash_reject_token_count"] = len(dflash_reject_token_indices)
            extra_fields["dflash_non_reject_token_count"] = max(
                len(token_ids) - len(dflash_reject_token_indices), 0
            )
            extra_fields["dflash_empty_reject_token_indices"] = int(
                len(token_ids) > 0 and len(dflash_reject_token_indices) == 0
            )
            if len(token_ids) > 0 and len(dflash_reject_token_indices) == 0:
                try:
                    self._write_dflash_outlier_log(
                        experiment_name=experiment_name,
                        request_id=request_id,
                        prompt_ids=prompt_ids,
                        token_ids=token_ids,
                        meta_info=meta_info,
                        reject_mask_len=len(dflash_reject_token_mask),
                        aligned_reject_mask_len=len(aligned_reject_token_mask),
                    )
                except Exception:
                    logger.exception("Failed to write DFLASH OPD outlier log for request %s.", request_id)
        for dflash_meta_key in (
    ...
    def _decode_token_ids(self, token_ids: Any) -> Optional[str]:
        tokenizer = getattr(getattr(self, "tokenizer_manager", None), "tokenizer", None)
    ...
    def _write_dflash_outlier_log(
        ...
        *,
        ...
        reject_mask_len: int,
        aligned_reject_mask_len: int,
    ) -> None:
        prompt_token_ids = prompt_ids.tolist() if hasattr(prompt_ids, "tolist") else list(prompt_ids)
        prompt_text = self._decode_token_ids(prompt_token_ids)
        response_text = self._decode_token_ids(token_ids)
        record = {
            "timestamp": time.time(),
            "global_steps": self.global_steps,
            "request_id": request_id,
            "reason": "empty_dflash_reject_token_indices",
            "finish_reason": meta_info.get("finish_reason"),
            "completion_tokens": meta_info.get("completion_tokens", len(token_ids)),
            "dflash_reject_token_mask_len": reject_mask_len,
            "dflash_reject_token_mask_aligned_len": aligned_reject_mask_len,
